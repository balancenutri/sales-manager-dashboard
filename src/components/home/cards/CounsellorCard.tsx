    ...
    <Card className="cursor-pointer hover:shadow-md transition-shadow">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between p-2 rounded-md bg-blue-50 text-blue-800 text-sm font-medium">
          ...
          <span>{data?.data.best_source_performance}</span>
        </div>
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-semibold">{data?.data.sales}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Revenue</p>
            <p className="font-semibold">{formatCurrency(Number(data?.data.revenue))}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Suggested Programs</p>
            <p className="font-semibold">{data?.data.suggested_programs}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Payment Details Shared</p>
            <p className="font-semibold">{formatCurrency(data?.data.suggested_amount)}</p>
          </div>
        </div>
