...
export default function AddCampaignForm({
  ...
  const [addNewCampaign] = useCreateAdPerformanceReportMutation();
  const [updateCampaign] = useUpdateAdPerformanceReportMutation();

  const onSubmit = async (formData: FormValues) => {
    const start = dayjs(formData.reporting_start);
    const end = dayjs(formData.reporting_end);

    if (end.isBefore(start)) {
      setError("reporting_end", {
        type: "manual",
        ...
        <Controller
          ...
          render={({ field }) => (
            <DatePicker
              ...
              value={field.value}
              onChange={field.onChange}
              errorMessage={errors.reporting_end?.message}
            />
          )}
        />
