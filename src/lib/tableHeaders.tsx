...
export const generateHeadersAndAccessors = (
  ...
  const columns: ColumnDef<any>[] = [
    {
      ...
      cell: ({ row }) => <div className="w-8 text-center font-medium">{row.index + 1}</div>,
    },
  ]
  ...
  const headers = Object.entries(datas.data[0]).reduce((acc, [key, value]) => {
    ...
    if (typeof value === "object" && value !== null) {
      const column: ColumnDef<any> = {
        id: key,
        accessorKey: key,
        header: () => <div className="text-xs font-semibold text-muted-foreground">{keyString(key).toUpperCase()}</div>,
        cell: ({ row }) => {
          ...
          switch (key) {
            case "user_details":
            case "client_details":
              return <UserDetailsCell value={cellValue} rowData={rowData} hideData={hideData} />
            // case "health_details":
            ...
            default:
              return <div className="text-xs">{JSON.stringify(cellValue)}</div>
          }
        },
      }
      acc.push(column)
  ...
  columns.push({
    ...
    cell: () => {
      return (
        <div className="py-2">
          <textarea placeholder="FU Notes" className="min-h-[80px] text-xs border rounded p-2 w-full" />
        </div>
      )
    },
  })
